...
The suite also includes data for the users to try options and ideas, and helper functions for plotting spectra using other R packages available from CRAN, in particular \pckg{ggplot2}. There are some packages, not part of the suite itself, for data acquisition from Ocean Optics spectrometers, and application of special calibration and correction procedures to those data. A future package will provide an interface to the TUV model to allow easy simulation of the solar spectrum.

\section{The suite}

The suite consists in several packages\index{r4photobiology!packages}. The main package is \pckg{photobiology} which contains all the generally useful functions, including many used in the other, more specialized, packages (Table \ref{tab:suite}).
...
\cran & ggspectra  & functions       & extensions to package \pckg{ggplot2} \\
\midrule
\midrule
