...
The same summary methods, are available for \texttt{\textunderscore spct} and \texttt{\textunderscore multi\textunderscore spct}, in the first case returning a vector, and in the second case, a \texttt{data.frame} object.
...
\begin{figure}
 ...
 \begin{tikzpicture}[%
  ...
  \end{tikzpicture}
  \caption[Object classes used in the packages]{Object classes used in the packages. Objects of \texttt{\textunderscore spct} classes are used to store spectra, in most cases a single spectrum. Objects of \texttt{\textunderscore multi\textunderscore spct} classes can be used to store \emph{collections} of \texttt{\textunderscore spct} objects, in most cases all belonging to the same class. Objects of class \texttt{waveband} contain information used for quantification: boundaries of a wavelength range and, optionally, spectral weighting functions. Gray-filled boxes represent classes defined in base R, yellow-filled boxes represent classes defined by contributed packages available through \gls{CRAN}, and blue-filled boxes represent classes defined in package \PB.}\label{fig:photobio:classes}
\end{figure}

<<suite-design-example, eval=FALSE>>=
e_irrad(sun.spct * polyester.new.spct, CIE())
@
...
Wavelengths should always be in naanometres (\Unit{nm}), and when conversion between energy and photon based units takes place no scaling factor is used (an input in \wattnm yields an output in \molnm rather than \umolnm).
