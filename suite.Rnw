...
\caption[Elements of the framework]{Elements of the framework used by all packages in the suite.}\label{fig:diagram:paradigm}
  ...
  \noindent
    \begin{description}
      ...
      \item[\texttt{\textunderscore mspct}] Containers for spectral objects are used to store related spectral objects, such as time series of spectral objects or spectral images.
...
The same summary methods, are available for \texttt{\textunderscore spct} and \texttt{\textunderscore mspct}, in the first case returning a vector, and in the second case, a \texttt{data.frame} object.
...
\cran & ggspectra  & functions       & extensions to package \code{ggplot2} \\
\midrule
\midrule
