...
The suite consists in several packages. The main package is \pckg{photobiology} which contains all the generally useful functions, including many used in the other, more specialized, packages (Table \ref{tab:suite}).
