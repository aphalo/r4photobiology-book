...
\begin{figure}
\caption{The elements of the suite.}\label{fig:diagram:paradigm}
  \begin{framed}
  \noindent
    \begin{description}
      \item[\texttt{\_spct}] Spectral objects are containers for different types of spectral data, data which is referenced to wavelength. These data normally originate in measurements or simulation with models.
