...
plot(CIE1924_lef.spct, annotations = c("colour.guide", "segments", "labels"), w.band = VIS_bands()) + xlim(350, 850)
plot(CIE1951_scotopic_lef.spct, annotations = c("colour.guide", "segments", "labels"), w.band = VIS_bands()) + xlim(350, 850)
@
\vspace{2pt}
\label{fig_eye}
...
plot(HJ_Blue.spct)
@
  \caption{Spectral irradiance for a blue LED array (Huey Jann, 50~W).}\label{fig:blue:led}
\end{figure}
...
@
  \caption{Spectral irradiance for `neutral white LED', 4000~K, array (Lumitronix SmartArray Q36 LED-Module, 39W, using Nichia 757 LEDs).}\label{fig:white:led}
\end{figure}
...
\section{Radiation interactions in tissues and cells}
