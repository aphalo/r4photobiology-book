...
res2 <- microbenchmark(
  ...
  times=100L, control=list(warmup = 10L))
@
\code{q\_irrad\_spct} without hinges enabled (the default when the wavelength step $\Delta \lambda < 1.1 \Unit{nm}$) takes \Sexpr{med1 <- median(res1$time) * 1e-6; signif(med1, 3)} ms but the execution time increases to \Sexpr{med2 <- median(res2$time) * 1e-6; signif(med2, 3)} ms, by \Sexpr{signif((med2 - med1)  / med1, 3)} times with use of hinges enabled.
