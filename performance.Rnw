...
res2 <- microbenchmark(
  ...
  times=100L, control=list(warmup = 10L))
@
When using \code{q\_irrad\_spct} with one waveband the time is \Sexpr{med1 <- median(res1$time) * 1e-6; signif(med1, 3)} ms and it increases to \Sexpr{med2 <- median(res2$time) * 1e-6; signif(med2, 3)} ms, by \Sexpr{signif(med2 / med1, 3)} times when with four wavebands.
...
res1 <- microbenchmark(
  ...
  times=100L, control=list(warmup = 10L))
@
When using \code{q\_irrad\_spct} time  is \Sexpr{med1 <- median(res1$time) * 1e-6; signif(med1, 3)} ms.
...
res2 <- microbenchmark(
  ...
  times=100L, control=list(warmup = 10L))
@
When using \code{q\_irrad\_spct} with cache enabled is \Sexpr{med1 <- median(res1$time) * 1e-6; signif(med1, 3)} ms and it increases to \Sexpr{med2 <- median(res2$time) * 1e-6; signif(med2, 3)} ms, by \Sexpr{signif(med2 / med1, 3)} times when with cacheing disabled.
...
res1 <- microbenchmark(
  ...
  times=100L, control=list(warmup = 10L))
res2 <- microbenchmark(
  q_irrad_spct(sun.spct, CIE(), use.cached.mult=TRUE, use.hinges=FALSE),
  times=100L, control=list(warmup = 10L))
@
When using \code{q\_irrad\_spct} with hinges enabled is \Sexpr{med1 <- median(res1$time) * 1e-6; signif(med1, 3)} ms and it decreases to \Sexpr{med2 <- median(res2$time) * 1e-6; signif(med2, 3)} ms, by \Sexpr{signif(med2 / med1, 3)} of the time used when with hinges enabled.
...
The differnces in the result are small.
...
res5 <- microbenchmark(
  ...
  times=100L, control=list(warmup = 10L))
@
When using operators with spectral objects execution time is \Sexpr{med1 <- median(res1$time) * 1e-6; signif(med1, 3)} ms and it decreases to \Sexpr{med2 <- median(res2$time) * 1e-6; signif(med2, 3)} ms, to \Sexpr{signif(med2 / med1 * 100, 3)} \% when using operators on vectors in a data frame, to \Sexpr{med3 <- median(res3$time) * 1e-6; signif(med3, 3)} ms, to \Sexpr{signif(med3 / med1 * 100, 3)} \% when using operators on vectors in a data table, to \Sexpr{med4 <- median(res4$time) * 1e-6; signif(med4, 3)} ms, to \Sexpr{signif(med4 / med1 * 100, 3)} \% when using operators on vectors in a \texttt{source.spct}, and to \Sexpr{med5 <- median(res5$time) * 1e-6; signif(med5, 3)} ms, to \Sexpr{signif(med5 / med1 * 100, 3)} \% when using data table syntax in a \texttt{source.spct}.
...
Rprof()
for (i in 1:1000) {
q_irrad_spct(sun.spct, my_PAR, use.hinges=TRUE)
}
Rprof(NULL)
summaryRprof()
@
...
Rprof()
for (i in 1:1000) {
q_irrad_spct(sun.spct, my_PAR, use.hinges=FALSE)
}
Rprof(NULL)
summaryRprof()
@
