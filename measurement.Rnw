...
Reading\index{foreign data!Ocean Optics spectrometers} spectral (energy) irradiance from a file saved in Ocean Optics SpectraSuite software , now superseded by OceanView.
...
.........
    ...
    \end{footnotesize}
  \end{framed}
  \caption[Top of text file \code{spectrum.SSirrad}]{Top of text file \code{spectrum.SSirrad} written by Ocean Optics' \pkg{SpectraView} software, and used in the example in section \ref{sec:OO:import}}\label{fig:listing:oo}
\end{figure}
