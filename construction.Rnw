...
from base R, should NOT be used. Package \PB provides function \texttt{rbinspct} for
...
Merging consists in merging different \emph{columns} from two spectra into a new combined spectrum. Another name for this type of operations, as used in package \texttt{dplyr}, is `join'. No wavelength interpolation is carried out, the two spectra must share wavelength values.


\section{Collections of multiple spectra}

\subsection{Task: Constructing \texttt{\textunderscore mspct} objects}

\subsection{Task: Retrieving \texttt{\textunderscore spct} objects from \texttt{\textunderscore mspct} objects}

\subsection{Task: Subsetting \texttt{\textunderscore mspct} objects}
