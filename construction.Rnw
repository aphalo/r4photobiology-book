...
For spectra the classes\index{spectral data!R classes} are a specialization of \code{tibble} which are derived from \code{data.frame}. This means that they are compatible with functions that operate on objects of these classes. See the documentation of package \pckg{tibble} for a description of the advantages of this class compared to base R's data frames.
...
In addition to the character strings \code{"second"}, \code{"hour"}, and \code{"day"}, any object belonging to the class \code{duration} defined in package \pckg{lubridate} can be used. This means, that any arbitrary time duration can be used.
...
`Traditional' conversion functions with names given by names of classes preceded by \code{as.} (e.g.\ \code{as.source\_spct}. These functions accept data frames, data tables, and lists with components of equal length as arguments. These functions are less flexible, as the component variables in the argument should be named using one of the names recognized. Table \ref{tab:obj:units} lists the different `names' understood by these constructor functions and the required and optional components of the different spectral object classes. The object is checked for consistency and within-range data, and missing required components are set to \code{NA}. We use \code{source\_spct} in the examples but similar functions are defined for all the classes spectral objects.
...
The \code{as.} functions, first make a copy of the data frame or data table, and then call one of the \code{set} functions described in section the section to convert the copy into a \code{\_spct} object. Table \ref{tab:obj:units} lists the different `names' understood by these copy functions and the required and optional components of the different spectral object classes. The new object is checked for consistency and within-range data, and missing required components are set to \code{NA}. We use \code{source\_spct} in the examples but similar functions are defined for all the classes spectral objects. In the same way as constructors, the \code{as.} functions accept attributes such as \code{time.unit} as arguments.
...
retains its class attributes, and other spectrum related attributes like the time unit, functions \code{rbind}
from base R, should NOT be used. Package \PB provides function \code{rbinspct} for
...
Merging consists in merging different \emph{columns} from two spectra into a new combined spectrum. Another name for this type of operations, as used in package \pckg{dplyr}, is `join'. No wavelength interpolation is carried out, the two spectra must share wavelength values.


\section{Collections of multiple spectra}

\subsection{Task: Constructing \code{\textunderscore mspct} objects}
