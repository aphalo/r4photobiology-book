...
For executing the examples listed in this chapter you need first to load the following packages from the library, the last three packages seem to interfere with each other, in particular GGally does not function if ggtern is loaded, so they are loaded only in the sections where they are used:
<<>>=
library(plyr)
library(grid)
library(Hmisc)
...
\begin{framed}
\noindent
Package GGally interferes with ggtern, and needs a session restart to get things back to normal. Detaching and unloading GGally is not enough to prevent the problems. Needs further investigation!
