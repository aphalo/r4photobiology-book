...
%%%
\section{Task: calculate daily effective energy exposure}\label{sec:wt:eirrad}
...
the output from the code above is in units of \jday, the code below returns the same result in the more common uints of \kjday:
