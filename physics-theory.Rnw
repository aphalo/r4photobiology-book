...
\chapter{Radiation interactions}\label{chap:spectroscopy}
...
%% Pedro
The attenuation of visible and \UV radiation by canopies is difficult to describe mathematically because it is a complex phenomenon. The spatial distribution of leaves is in most cases not uniform, the display angle of the leaves is not random, and may change with depth in the canopy, and even in some cases with time-of-day. Here we give only a description of the simplest approach, the use of an approximation based on Beer's law as modified by \citeauthor{Monsi1953} \autocite*{Monsi1953}, reviewed by \citeauthor{Hirose2005} \autocite*{Hirose2005}. Beer's law (Equation \ref{eq:Beer-Lambert}) assumes a homogeneous light absorbing medium such as a solution. However, a canopy is heterogenous, with discrete light absorbing objects (the leaves and stems) distributed in a transparent medium (air).
\begin{equation}
...
sometimes increase \UV reflectance. The diffuse component of \UV radiation is proportionally larger than that
of visible light (Figure \ref{fig:diffuse:UV}).
...
tends to increase \autocite[see][]{Brown1994}. In contrast, \citeauthor{Deckmyn2001} \autocite*{Deckmyn2001} observed a decrease in \UVPAR ratio in white clover canopies with planophyle leaves. \citeauthor{Allen1975} \autocite*{Allen1975} modelled the
\UVB penetration in plant canopies, under normal and depleted ozone conditions.
\citeauthor{Parisi1996} \autocite*{Parisi1996} measured
...
foliage, are given by \citeauthor{Campbell1998} \autocite*{Campbell1998} and \citeauthor{Monteith2008} \autocite*{Monteith2008}.
...
\subsection{Specular and total reflectance}

\subsection{Internal and total transmittance}

\subsection{Absorbance and absorptance}
