...
It is also possible to use \code{data.table} syntax as follows, as in addition to the \code{T2A} method for \code{filter\_spct} there is a \code{T2A} method available for numeric vectors.
...
Please, see section \ref{sec:trans:T2A} for more details in the description of the method \code{T2A} which does the opposite conversion than the method \code{A2T} needed for this taks, but which works similarly.
