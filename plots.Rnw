...
We can also add one colour bar as key. We here prepare data for an almost continuous colour reference guide, one for colours perceived by plants and one for ISO colour definitions. These are quick and dirty examples for the discrete bands, but they get the job done. A better approach would be just to generate the data for the rectangles with function  \code{tag}, say if supplied only with a list of wavebands as argument, because having a single rectangle for each waveband would allow setting the border of rectangle as needed, or just plot empty rectangles with text labels inside.

<<>>=
x <- seq(278.5,798.5, length.out=200)
wl.guide.data <- data.frame(w.length = x, s.e.irrad = NA,
                            xmin = x - 1.5, xmax = x +1.5, ymin = 0.85, ymax = 0.9)
setGenSpct(wl.guide.data)
plant.guide.data <- copy(wl.guide.data)
iso.guide.data <- copy(wl.guide.data)
tag(wl.guide.data, split_bands(wl.guide.data, length.out=200))
tag(plant.guide.data, Plant_bands())
tag(iso.guide.data, VIS_bands())
@

The continuous colour bar.

<<>>=
fig_sun.yy <- fig_sun.y +
geom_rect(data=wl.guide.data,
       aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
           y = 0, fill=wb.f),
       color = NA) +
  scale_fill_tgspct(tg.spct=wl.guide.data)
fig_sun.yy
@

We just replace the data, both in \code{geom\_rect} and in \code{scale\_fill\_tgspct} for the iso colours,

<<>>=
fig_sun.yyy <- fig_sun.y +
geom_rect(data=iso.guide.data,
       aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
           y = 0, fill=wb.f),
       color = NA) +
  scale_fill_tgspct(tg.spct=iso.guide.data)
fig_sun.yyy
@

or the plant's sensory colours.

<<>>=
fig_sun.yyy <- fig_sun.y +
geom_rect(data=plant.guide.data,
       aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
           y = 0, fill=wb.f),
       color = NA) +
  scale_fill_tgspct(tg.spct=plant.guide.data)
fig_sun.yyy
@
