...
If you are not familiar with package \pkg{ggplot2} and plotting using the \emph{grammar of graphics}, please read the chapter on plotting in the book \emph{Learning R \ldots{}as your learnt your mother tongue} \autocite{Aphalo2016}.

%%%%
\section{Using \texttt{autoplot()} methods with spectra}
...
\begin{framed}
\noindent
As the current implementation uses \code{ggplot} `statistic's, waveband irradiance annotations respect global aesthetics and facets. If used for simultaneous plotting in the same panel of several spectra (stored \emph{longitudinally} in a single R object), then the \emph{summaries} annotation will be disabled by default and should not be added as the values would be over-plotted and ambiguous.
\end{framed}

While the argument passed to \code{range} is used to trim the spectrum before plotting using interpolation if necessary, the argument passed to \code{ylim} is used to set the limits of the $y$ scale of the \code{ggplot} object.

\subsection{Task: plotting of normalized \code{source\_spct} objects}\label{sec:plot:source:spc}

Normalization consists in scaling a spectrum so that the spectral quantity takes a given value, usually one, at a certain wavelength. See section \ref{sec:algo:normalization} on page \pageref{sec:algo:normalization} for a more detailed explanation.

<<plot-sun-normalize-0>>=
autoplot(normalize(sun.spct))
@

<<plot-sun-normalize-1>>=
autoplot(normalize(sun.spct, norm = "max"))
@

<<plot-sun-normalize-2>>=
autoplot(normalize(sun.spct, norm = 700))
@

<<plot-sun-normalize-3>>=
autoplot(normalize(sun.spct, norm = "max"), range = VIS())
@

<<plot-sun-normalize-4>>=
...
<<plot-object-1>>=
autoplot(Solidago_altissima.mspct$upper_adax)
@

The default for \code{plot.qty} is \code{"all"} (meaning Rrf, Tfr, and Afr).
<<plot-object-2>>=
...
@
<<plot-object-4>>=
autoplot(Solidago_altissima.mspct$upper_adax, plot.qty = "absorptance")
@
<<plot-object-5>>=
autoplot(Solidago_altissima.mspct$upper_adax, plot.qty = "transmittance")
@
<<plot-object-6>>=
autoplot(Solidago_altissima.mspct$upper_adax, plot.qty = "absorbance")
@

As for \code{filter\_spct} and \code{reflector\_spct} quantities can be plotted as percentages. We give only one example here.

<<plot-object-7>>=
autoplot(Solidago_altissima.mspct$upper_adax, pc.out = TRUE)
@

For details on how to modify the wavebands used for labels and summaries, and customization of annotations, please see the previous section \ref{sec:plot:source:spc} on page \pageref{sec:plot:source:spc}.

\subsection{Task: plotting collections of spectra}\label{sec:plot:generic:mspct}

The \code{autoplot()} method described above also have specializations for collections of spectra. By default they create a plot with a single panel and one line per spectrum.

<<plot-mpsct-01>>=
autoplot(filters.mspct[c("Schott_UG1", "Schott_GG435")], range = c(280, 800))
@

These methods support plotting using one panel per spectrum in the collection.

<<plot-mpsct-02>>=
autoplot(filters.mspct[c("Schott_UG1", "Schott_GG435")], range = c(280, 800), facets = TRUE)
@

The data from the spectra can be combined on-the-fly. For example, we can plot the approximate transmittance of a stack of two filters. For this we need to plot the product of the transmittances.

<<plot-mpsct-03>>=
autoplot(filters.mspct[c("Schott_UG1", "Schott_BG39")], range = c(280, 800), plot.data = "prod")
@

These methods can also be used with collections of spectra created on-the-fly by combining other spectra as described in Chapter \ref{chap:baseoper} starting on page \pageref{chap:baseoper}.
...
%%%%
\section[Plotting spectra with ggplot]{Plotting spectra with \code{ggplot}}\label{sec:plot:spc}
...
\begin{framed}
\ilAttention\index{conflict with \pkg{ggtern}}\noindent If package \pkg{ggtern} is loaded simultaneously with package \pkg{ggmisc} the examples in this chapter will give an error. The conflict originates in that \pkg{ggtern} redirects calls to method \code{ggplot} to itself, and does not dispatch the specialized methods defined in \pkg{ggpmisc}. The solution is to give the name of the namespace explicitly as shown below.
...
\subsection{Task: plotting a spectrum as discrete columns}
...
We use function \code{multiplot} to make a single plot from two separate ggplot objects, and put them side by or on top of each other. We use different $y$-axis labels in the two cases to make better use of the available space.
...
<<plot-peaks-valleys-6, eval=good_label_repel>>=
ggplot(sun.spct) +
  geom_line() +
...
\begin{table}
  ...
  \begin{tabular}{lp{0.68\linewidth}}
    \toprule
    \emph{statistic} & \emph{description} \\
    \midrule
    \emph{Graphic elements} & \\
    \midrule
    stat\_wb\_box & Draw a box showing wavelength range and color \\
    stat\_wb\_column & Draw a column with area equal to integral and color \\
    stat\_wb\_hbar & Draw a bar showing mean spectral irradiance, wavelength range and color\\
    stat\_wl\_strip & Draw bar with colours based wavelength (nm) or wavebands \\
    \midrule
    \textbf{Summary labels} & \\
    \midrule
    ...
    stat\_wb\_label & Label with name of wavelength ranges or wavebands \\
...
Here is an example for \PAR using defaults except for \code{w.band} and \code{color}.
...
Now an example that is more complex, and demonstrates the flexibility of plots produced with \pkg{ggplot2}. We add annotations for all the colors defined in the ISO standard. For each one we use, as above, two \emph{geoms}. We can also see in this example that the annotations look nicer on a white background, which can be obtained with \code{theme\_bw}. In this case, the
effect of automatic selection of black or white text is important.
...
We change aesthetics to obtain dashed lines, and add a label using \code{stat\_wb\_label} as described above.
...
We use photon based units for expressing both the spectral irradiance and the integral.
...
With the two-spectra data object, we use \emph{facets} to plot each spectrum on a separate panel. By default, the $y$-scale is common to all panels.
<<>>=
...
We will first show how to generate the colour information from wavelengths by means of \emph{statistics} defined in \pkg{ggspectra} and later give some examples of using colors by combining functions from packages \pkg{photobiology} and \pkg{ggplot2}.
\code{stat\_wl\_summary()} and \code{stat\_wb\_mean()} return a \emph{reduced} data set with fewer rows than the original data. \code{stat\_wl\_strip()} depending on the input, can also return a data set with more rows than the input data. This is the default behaviour, with \code{w.band} with argument \code{NULL}. This \emph{stat} operates only on the variable mapped to the $x$ \emph{aesthetic}, a $y$-mapping is not required as input. The \emph{stats} with \code{wb} in their name have a parameter \code{w.band} to which \code{waveband} objects or lists of \code{waveband} objects can be passed as arguments, while \emph{stats} with \code{wl} in their name have also a parameter \code{range} to which a \code{numeric} vector of length two can be passed, or any R object on which function \code{range()} returns such a vector, interpretable as a range of wavelengths expressed in nanometres.
...
fig_sun.cie0 <-
  ggplot(data = sun.eff.cie.spct,
         aes(x = w.length, y = s.e.irrad, fill = filter)) +
  scale_fill_grey() +
  geom_spct() +
  labs(x = xlab_nm,
...
q.irrad.sun.spct <- tibble(q.irrad = q.irrad.bands.sun,
                           w.length = w.length,
                           wb.color = wb.color,
                           wb.name = wb.name)
@
...
%rgb.values <- col2rgb(color_of(w.lengths))
