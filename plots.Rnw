...
ggplot(pl.sun.spct,
       aes(x=w.length, y=s.e.irrad)) +
  scale_fill_identity(na.value=NA) +
  geom_area(aes(fill=as.character(wb.color))) +
  geom_line() +
  labs(
    y = expression(Spectral~~energy~~irradiance~~(W~m^{-2}~nm^{-1})),
    x = "Wavelength (nm)")
...
ggplot(pl.sun.spct,
  ...
  scale_fill_grey(na.value=NA, name="", labels=c("UVB", "UVA", "Blue", "Green", "Red", "Far red")) +
  geom_line() +
  labs(
    y = expression(Spectral~~energy~~irradiance~~(W~m^{-2}~nm^{-1})),
    x = "Wavelength (nm)")
...
scale_fill_wavelength <-
  function(...,
    ...
    values=as.character(color(split_bands(tg.spct, length.out=length(levels(tg.spct$wb.f)))))
    ggplot2:::manual_scale("fill", values, guide = guide, na.value = NA, ...)
}
@
...
scale_colour_wavelength <-
  function(...,
           ...
           guide = "none", na.value=NA) {
    values=as.character(color(split_bands(tg.spct, length.out=length(levels(tg.spct$wb.f)))))
    ggplot2:::manual_scale("colour", values, guide = guide, na.value = NA, ...)
}
@
...
ggplot(splt.sun.spct,
       aes(x=w.length, y=s.e.irrad)) +
  scale_fill_wavelength(tg.spct=splt.sun.spct) +
...
ggplot(splt1.sun.spct,
       aes(x=w.length, y=s.e.irrad)) +
  scale_fill_wavelength(tg.spct=splt1.sun.spct) +
  geom_area(aes(fill=wb.f), alpha=0.8) +
  geom_line() +
  labs(
    y = expression(Spectral~~energy~~irradiance~~(W~m^{-2}~nm^{-1})),
    x = "Wavelength (nm)")
...
ggplot(splt1.sun.spct,
       aes(x=w.length, y=s.e.irrad)) +
  scale_colour_wavelength(tg.spct=splt1.sun.spct) +
  geom_line() +
