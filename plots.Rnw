...
application specific functionality on separate packages. It uses the grammar
...
<<>>=
fig_sun + stat_peaks(span=41, ignore_threshold=1e-2) +
...
<<>>=
fig_sun + stat_peaks(colour="red", span=51) +
...
Now an example that is more complex, and demonstrates the flexibility of plots produced with \code{ggplot2}. We add annotations for eight different wavebands, some of them overlapping. For each one we use two `geoms' and some labels are rotated and justified to keep them centred.

<<fig.width=8, fig.height=6, out.width='.95\\textwidth'>>=
figv2 <- fig_sun +
  annotate_waveband(UVC(), "rect", ymax=0.82) +
