...
If you are not familiar with \pkg{ggplot2} and \code{ggtern} plotting, please read Appendix
...
Being a \pkg{ggplot2}-compatible \code{stat\_peaks} and \code{stat\_valleys} accept a \code{geom} argument
...
THIS SECTION NEEDS TO BE RE-WRITTEN USING CURRENT VERSION OF \pkg{ggspectra}.
...
And one where we change some of the aesthetics in the previous figure, and add a label using \code{stat\_wb\_label} defined in package \pkg{ggspectra}.
...
<<plot-cie-filter-sun, warning=my.debug, eval=FALSE>>=
fig_dsun <-
...
Here we simply use the \code{wb.f} factor that was added as part of the tagging, with the default colour scale of \pkg{ggplot2}, which results in a palette unrelated to the real colour of the different wavelengths.
...
We can also use other \code{geom}s like \code{geom\_area} in the next chunk, together with, as an example, a grey fill scale from \pkg{ggplot2}.
