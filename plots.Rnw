...
fig_sun <- ggplot(data=sun.spct,
  ...
  geom_line() +
  labs(y = expression(Spectral~~energy~~irradiance~~(W~m^{-2}~nm^{-1})),
       x = "Wavelength (nm)")
...
We add colour and waveband information to the spectral data object.
...
Here we use the \code{wb.f} factor that was added as part of the tagging.
...
We add colour and waveband information to the spectral data object, but before that we increase the number of `observations' in the spectrum by interpolation (here commented out). You can play with \code{length.out} and the \code{size} of points until you get the result you like. We here tag the solar spectrum with human vision colours as defined by CIE.

<<>>=
tg.sun.spct <- copy(sun.spct)
# tg.sun.spct <- interpolate_spct(tg.sun.spct, length.out=1000)
tag(tg.sun.spct, VIS_bands())
@

Here we plot using colours by waveband using the colour definitions by CIE, with symbols filled with colours. The colour data outside the wavebands is set to \code{NA} so those points are not filled.
...
Using \code{geom\_area} we can fill the area under the curve according to the colour of different wavebands, we set the fill only for this geom, so that the \code{NA}s do not affect other ploting. To get a single black curve for the spectrum we use \code{geom\_line}.

<<>>=
fig_sun.zz <-
ggplot(tg.sun.spct,
       aes(x=w.length, y=s.e.irrad)) +
  geom_area(aes(fill=wb.color), alpha=0.5) +
  scale_fill_identity() +
  geom_line() +
  labs(
    y = expression(Spectral~~energy~~irradiance~~(W~m^{-2}~nm^{-1})),
...
Here we plot using colours by waveband using the defineitions of plant sensory `colours'.
...
We can also use the factor \code{wb.f} which has value \code{NA} outside the wavebands, changing the colour used for \code{NA} to \code{NA} which renders it invisible.
...
When using a factor we can play with the scale definitions and represent the wavebands in any way we may want.

We may instead want to plot the colour calculated for each individual wavelength value, which \code{tag} adds to the spectrum, whether a waveband definition is supplied or not.
...
<<>>=
detach(package:photobiologygg)
detach(package:ggtern)
detach(package:ggplot2)
detach(package:gridExtra)
detach(package:photobiologyFilters)
