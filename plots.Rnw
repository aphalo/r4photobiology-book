...
We add colour and waveband information to the spectral data object, but before that we increase the number of `observations' in the spectrum by interpolation. You can play with \code{length.out} and the \code{size} of points until you get the result you like.
...
Here we plot using colours by waveband.
