...
filled highlights the total energy irradiance (or photon irradaince) for a given region
...
interpretation on wavelengths expressed in namometres. Labeling regions of the spectrum
...
Here without need to recreate the figure, we add a logarithmic scale for the $y$-axis and print on the fly the result, and two of the just saved axis-labels. In this case we override the automatic limits of the scale. We do not give further exampls of this, but could be also used with later examples, just by adjusting the values used as scale limits.
...
We can use function \code{grid.arrange} to make a single plot from two separate ggplots, and put them side by or on top of each other. We use different $y$-axis labels in the two cases to make better use of the avaialble space.
...
When we want to colour-label individual spectral values, for example, by plotting the individual data points with the colour corresponding to their wavelengths, or fill the area below a plotted spectral curve with colours, we need to first \code{tag} the spectral data set using a waveband defintion or a list of waveband definitions. If we just want to add a guide or labels to the plot, we can create new data instead of tagging the spectral data to be plotted. In section \ref{sec:plot:tag} we show code based on tagging spectral data, and in section \ref{sec:plot:guide} the case of using different data for plotting the guide or key is described.

\subsection{Scale definitions}\label{sec:plot:scales}

First we define some new scales for use for plotting with \code{ggplot} when plotting wavelength derived colours. In the future something equivalent may be included in package \code{photobiologygg} as predifined scales. We define two very similar scales, one for colour, and one for fill aesthetics.
...
The default fill looks too dark and bold, so we change the transparency of the fill by setting \code{fill = 0.3}. The grid in the background becomes slighlty visible also in the filled region, facilitating `reading' of the plot and avoiding a to stark contrast bewteen regions, which tends to be disturbing. In later plots we frequently use \code{alpha} to improve how plots look, but we examplify the effect of changing this aesthetic only here.
...
Using \code{geom\_area} we can fill the area under the curve according to the colour of different wavebands, we set the fill only for this geom, so that the \code{NA}s do not affect other ploting. To get a single black curve for the spectrum we use \code{geom\_line}. This approach works as long as wavebands do not share the same value for the color, which means that it is not suitable either wwhen more than one band is outside the visible range, or when using many narrow wavebands.
...
We can also use the factor \code{wb.f} which has value \code{NA} outside the wavebands, changing the colour used for \code{NA} to \code{NA} which renders it invisible. We can change the labels used for the waavebands in two different way, when plotting by suppying a labels argument to the scale used, or when tagging the spectrum. The second approach is simpler when producing several different plots from the same spectral object, or when wnating to have consitent labels and names used also in derived results such as irradiances.
...
When ploting points, rather than an area we may, instead of using colours from wavebands, want to plot the colour calculated for each individual wavelength value, which \code{tag} adds to the spectrum, whether a waveband definition is supplied or not. In
...
Function \code{wb2tagged\_spct} returns the same `spectrum', but tagged with the same wavbands as used to create the spectral data, and you will also notice that a `hinge' has been added, which is redundant in the case of a single waveband, but needed in the case of wavebands sharing a limit.
...
This second example differs very little from the previous one, but by using a waveband definition instead of a spectrum as argument to \code{split\_bands}, we restrict the region covered by the colour fill to that of the waveband. In fax a vector of length two, or any object for wich a \code{range} method is available can be used as input to this function.
...
To desmonstrate this we replace in the previous example, the previous tagged spectrum with one based on ISO colours. We need to do this replacement in the calls to both \code{geom\_rect} and \code{scale\_fill\_tgspct}.
...
We can also use \code{geom\_segment} to draw lines, including arrows. In this example we also set a different font \code{family} and label text. We can replace the label text which is by default obtained from the waveband defintion by assigning a name to the waveband as member of the list. We use single quotes so that the long name containing space characters is accepted by \code{list}.
...
Function \code{wb2tagged\_spct} returns the same data as \code{wb2spct} but `tagged'. Aas shown in the next code chunk, tagging allows us to use waveband-dependent colours to the vertical lines.
...
\section{Task: plotting effective spectral irardiance}
...
There is one warning issued for each panel, as the use of \code{xlim} discards 400 observations for wavelengths longer than 400~\Unit(nm). One should be aware that these are estimated values and in practice stray light reduces the effectivity of the filters for blocking radiation, and the amount of stray light depends on many factors including the relative positions of plants, filter and sun.
...
\section{Task: making a bar plot of effective irradiances}

<<echo=FALSE, include=FALSE>>=
opts_chunk$set(opts_fig_narrow)
@

In this task we aim at creating bar plots depicting the contributions of the UVB and UVA bands to the total erythemal effective irradiance in sunlight filtered with different palstic films. First we calculate the effective energy irradiances using the waveband definition for erythemal BSWF (CIE98) separately for the estimated solar spectral irradiance under each filter type.
...
We assemble a data table by concatenating the irradiances and adding factors for filter type and wave bands. When
deffining the factors, we use \code{levels} to make sure that the levels are ordered as we would like to plot them.
...
Now we plot stacked bars using \code{geom\_bar}, however as the default \code{stat} of this geom is not suitable for our data, we specify \code{stat="identity"} to have the data plotted as is. We set a specific pallete for fill, and add a black border to the bars by means of \code{color="black"}, we remove the grid lines corresponding to the $x$-axis, and also position the legend within the plotting region.
...
We show now the last example, related to the ones above, but creating a bar plot with more bars. First we calculate photon irradiances for different equally spaced bands within PAR using function \code{split\_bands}. The code is written so that by changing the first two lines you can adjust the output.
