...
Objects of class \code{response.spct} have two mandatory components \code{w.length}, and \code{s.e.response} giving the spectral response to energy or \code{s.q.response} giving spectral quantum response. The response units can be volts or amperes for instruments and sensors, or a biological response for organisms.
...
\midrule
reflector.spct & w.length & --- & wl, wavelength & nm \\
            & Rfr      & --- & --- & $x/1$  \\
            & ---      & Rpc    & reflectance & \% \\
\midrule
...
As noted above operators are available for \code{generic.scpt}, \code{source.spct}, \code{filter.spct} and \code{reflector.spct} objects, and `recycling' takes place when needed:
...
Among spectra of the same type, all normal arithmetic operations and some functions are allowed, and return a spectrun of the same type. However, when an operation between spectra of two different types makes no sense from the physics or optics point of view, \code{NA} is returned. The returned type depends also on the physical expectations. For example the multiplication (or convolution) of a light source spectrum \code{source.spct} and the transmittance spectrum of a filter \code{filter.spct} yields a new light source \code{source.spct} object. In contrast the addition of a \code{source.spct} and a \code{filter.spct} yields \code{NA}, a missing value, as such an operation is \emph{undefined}.
