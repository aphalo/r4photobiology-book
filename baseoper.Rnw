...
Package \pckg{phobiologyFilters} makes available many different filter spectra, from which we choose Schott filter GG400. Package \pckg{photobiology} makes available one example solar spectrum. Using these data we will simulate the filtered solar spectrum.
...
Other options affect the optimization of performance vs.\ precision of calculations and can be useful especially when processing huge numbers of spectra. Some options defined in base R and package \pckg{dplyr} affect printing of output (Table \ref{tab:options}).
