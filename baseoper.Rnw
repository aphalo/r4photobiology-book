...
<<>>=
sun.spct * filters.mspct$GG400 * (100 - 9) / 100
@
...
Function \code{q2e} is the reverse of \code{e2q}, converting spectral photon irradiance in \molnm to spectral energy irradiance in \wattnm . It can also be used as a roundabout way of removing a \code{s.q.irrad} column, which could be useful in some cases.
...
photobiology.filter.qty & \code{"transmittance"} & output ($/1$) \\
                        ...
                        & \code{"absorbance"} & output (a.u. $\log_{10}$ base) \\
photobiology.use.hinges & \code{NULL} & guess automatically \\
                        & \code{TRUE} & do insert hinges \\
                        & \code{FALSE} & do not insert hinges \\
