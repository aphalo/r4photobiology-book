...
In R vectorized operators are available for "generic.scpt", "source.spct", and "filter.spct" objects, and `recycling' takes places when needed\footnote{Here and in many other examples \code{head} is used to limit the ammount of output to a few lines.}:
