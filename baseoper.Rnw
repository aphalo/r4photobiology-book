...
\subsubsection{Task: Using options to change default behaviour of maths operators and functions}
...
Other options affect the optimization for performance vs.\ precision of calculations and can be useful especially when processing huge numbers of spectra. Some options defined in base R and package \texttt{data.table} affect printing of output.
