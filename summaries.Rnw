...
<<>>=
peaks(macam.mspct$philips.tl12)
peaks(macam.mspct$philips.tl12, unit.out = "photon")
peaks(macam.mspct$philips.tl12, span = 50)
@
...
<<>>=
stepsize(oo_maya.mspct$germicidal)
peaks <-
   with(oo_maya.mspct$germicidal,
...
ggplot(data = fitted_peak.spct, aes(w.length, s.e.irrad)) +
  ...
  geom_point(data = oo_maya.mspct$germicidal, colour = "red", size = 3) +
  xlim(250, 260)
