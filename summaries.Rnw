...
Methods \texttt{names} and \texttt{comment} should work as usual. In the case of the comment attribute, most operations on spectral objects preserve comments, sometimes with additions, or by merging of comments from operands. Comments are optional, so for some objects \texttt{comment} may return a \texttt{NULL}. As some comments contain new line characters, to get them printed nicely we need to use \texttt{cat}.
