...
\chapter{R Scripts and Programming}
...
Any result that needs to be made available outside the funtion must be returned by the function. If the function \texttt{return} is not explicitly used, the value returned by the last statement within the body of the function will be returned.
