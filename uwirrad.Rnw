...
%%%%
\section{Task: (energy) irradiance from spectral irradiance}\label{sec:energy:irrad}

The task to be completed is to calculate the (energy) irradiance ($\irr$) in \watt from spectral (energy) irradiance ($\spfd$) in \wattnm and the corresponding wavelengths ($\lambda$) in nm.

\begin{equation}
\pfd[\lambda_1 < \lambda < \lambda_2] = \int_{\lambda_1}^{\lambda_2} \sirr\ \mathrm{d}\ \lambda
...
Function \code{PAR()} is predefined in package \PBVIS as a convenience function, so the code above can be replaced by:
...
%%%%
\section{Task: photon irradiance from spectral irradiance}\label{sec:photon:irrad}
...
%%%%
\section[Task: irradiance from spectral photon irradiance]{Task: calculate energy and photon irradiances from spectral photon irradiance}
...
%%%%
\section{Task: irradiances for more than one waveband}
...
For example a ratio frequently used in plant photobiology is the read to far-red photon ratio (R:FR photon ratio or $\zeta$). If we follow the wavelength ranges in the definition given by \cite{Morgan1981}, using photon irradiance\footnote{In the original text photon fluence rate is used but it not clear whether photon irradiance was meant instead.}:
...
When comparing photo-chemical and photo-biological responses under different light sources it is of interest to calculate the photons per energy in \molwatt. In this case only one waveband definition is used to calculate the quotient:
...
For obtaining the same quotient in \umolwatt we just need to multiply by $10^6$. We can use such a multiplier to convert $\irr$ [\watt] into $\pfd$ [\umol], or as a divisor to convert $\pfd$ [\umol] into $\irr$ [\watt], \emph{for a given light source and waveband}:
...
<<>>=
with(sun.data,
     split_energy_irradiance(w.length, s.e.irrad, c(400, 500, 600, 700)))
@
...
<<>>=
with(sun.data,
     sum(split_energy_irradiance(w.length, s.e.irrad, c(400, 500, 600, 700))))
with(sun.data,
...
<<>>=
with(sun.data,
     split_energy_irradiance(w.length, s.e.irrad, c(400, 500, 600, 700), scale="relative"))
@
...
<<>>=
with(sun.data,
     split_energy_irradiance(w.length, s.e.irrad, c(400, 700), scale="percent"))
@

Although for \code{scale="absolute"}, the default, it can be used as a quick way of calculating an irradiance for a range of wavelengths without having to define a \code{waveband}:

<<>>=
with(sun.data,
     split_energy_irradiance(w.length, s.e.irrad, c(400, 700)))
@
