...
\sloppy
It can be of interest to split the total (energy) irradiance into adjacent regions delimited by arbitrary wavelengths. 
When working with \code{source.spct} objects, the best way to achieve this is to combine the use of the functions \code{e\_irrad} and \code{split\_bands} already described above, for example,
