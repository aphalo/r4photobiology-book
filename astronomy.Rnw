...
We\Attention{time zones} should make explicit the time zones used for both input and output to avoid any ambiguities.
...
Functions \code{day\_length} and \code{night\_length} have same parameter signature. They are vectorised for the \code{date} parameter.

Calculating the length of the current day is easy. We use the Greenwich meridian, and 60 degrees in the Northern and Southern hemispheres.
\code{today}.
<<>>=
...
Or if we give a date explicitly using functions from package \pckg{lubridate}.
...
Functions \code{sunrise\_time}, \code{sunset\_time}, and \code{noon\_time} have the same parameter signature---take the same arguments---but the values they return differ.
...
Both\Attention{locale} latitude and longitude can be supplied, but be aware that if the returned value is desired in the local time coordinates, the time zone should match the longitude.
...
Default latitude is zero (the Equator), the default longitude is zero (Greenwich), and default time zone for the functions in the \pckg{photobiology} package is \code{"UTC"}. The default for \code{date} is the current day in time zone UTC. Using defaults the code is simpler, but this is not a good approach for scripts.
...
\code{sun\_angles} not only returns solar elevation, it returns all the angles defining the position of the sun. The time argument to \code{sun\_angles} is internally converted to UTC (universal time coordinates, which is equal to GMT) time zone, so time defined for any time zone is valid input. The time zone used by default for the output can vary as described in page \pageref{alert:time:zone}, so it is more reliable specify the time coordinates used for the output with parameter \code{tz}, using arguments valid for pacakge \code{lubridate}---which is used internally by package \pckg{photobiology}.
...
For this we reuse \code{days} from the previous sections. We repeat the calculations for three locations at different latitudes, then row bind the data frames into a single data frame. Data frames contain information to identify the sites:
...
By replacing \code{sunrise\_time} by \code{sunset\_time} in the code above and editing the axis label of the plot one can produce a similar plot of sunset times.
