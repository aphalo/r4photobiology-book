...
\begin{framed}
\ilAttention{} Both latitude and longitude can be supplied, but be aware that if the returned value is desired in the local time coordinates, the time zone should match the longitude.
\end{framed}
