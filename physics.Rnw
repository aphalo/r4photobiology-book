...
\end{equation}
%
with Boltzmann's constant $k_\mathrm{B}=1.381\times 10^{-23}$ \Unit{J\,K^{-1}}, Planck constant\index{Planck constant} $h=6.626\times 10^{-34}$ \jsecond and speed of light in vacuum $c=2.998\times 10^{8}$ \msecond.
...
We aware that if two vectors are supplied, then the elements in each one are matched and recycled\footnote{Excercise: calculate each of the four values individually to work out how the two vectors are being used.}:
...
Given this simple relation $\tau(\lambda)$ can be calculated as a division between two "source.spct" objects. This gives the correct answer, but in an object of class "source.scpt".

<<eval=FALSE>>=
NULL
@

Absopttance is just $1 - \tau(\lambda)$, but should be distinguished from absorbance ($A(\lambda)$) which is measured on a logarithmic scale:
...
They could be directly applied to spectra but doing this still gives "filter.spc" objects as a result, with the data labelled "T", when it has change inot Absorbance. As the spectra objects are data.tables, one can add a new column, say with transmittances to a copy of the filter data as follows.
