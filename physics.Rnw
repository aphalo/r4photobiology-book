...
They could be directly applied to spectra but doing this still gives "filter.spc" objects as a result, with the data labelled "T", when it has change into absorbance. As the spectra objects are data.tables, one can add a new column, say with transmittances to a copy of the filter data as follows.
